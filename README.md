...
这是一个可直接容器化部署的个人情感陪伴助手：本地 `Qwen3-Embedding-0.6B`（TEI）负责向量化，Neo4j 同时保存向量、记忆节点和图谱关系；便宜模型筛选长期记忆、抽取情绪，主模型负责对话与工具调用；腾讯官方 BotGo 桥接负责与 QQ 私聊（C2C）通信。Neo4j、TEI、AI API、QQ 桥接四个组件打包在**同一个镜像**里，填好 `.env` 一条命令即可启动。
...
- **双模型记忆**：便宜模型自动筛选值得长期记住的内容（纯寒暄短消息自动跳过省 token），主模型对话并按需调用记忆工具。
...
| `EMBEDDING_BASE_URL` | `http://127.0.0.1:8080` | Embedding 接口地址（TEI 与应用同容器，回环地址） |
...
| `NEO4J_URI` | `bolt://127.0.0.1:7687` | Neo4j Bolt 地址（Neo4j 与应用同容器，回环地址） |
...
- `(User)-[:HAS_CONVERSATION]->(Conversation)-[:HAS_MESSAGE]->(Message)` 保存短期对话历史（Message 带自增 `seq` 保证顺序确定；Conversation 上的 `summary` 滚动压缩更早的对话）；
...
- CPU 部署：建议至少 4 核、8 GB RAM；内存较小时先把 `EMBEDDING_CONTEXT_SIZE=4096`，并按 `.env.example` 里的注释调低 Neo4j 堆内存。
- 磁盘：建议预留至少 8–10 GB 给镜像、模型缓存和数据库。
- 镜像内置的是 TEI CPU 推理；如需 GPU 推理，可在外部单独运行 TEI CUDA 容器并把 `EMBEDDING_BASE_URL` 指过去。
...
`main` 分支通过测试后，GitHub Actions 会发布一个 `linux/amd64` 镜像（Neo4j、TEI、AI API、QQ 桥接全部合并在同一镜像内）：
