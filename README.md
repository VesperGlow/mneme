...
这是一个可直接容器化部署的个人情感陪伴助手：**一个 Rust 单二进制**（axum + rusqlite + ort）内完成一切——进程内跑 uint8 量化的 `Qwen3-Embedding-0.6B` 与 `Qwen3-Reranker-0.6B`（ONNX）做向量化与二段精排，SQLite 单文件保存对话、长期记忆与情绪时间线；记忆**追加式永久保留、不按时间遗忘、不再分级**，便宜模型只判「记不记」并抽取情绪，主模型负责对话与工具调用；QQ 桥接按官方开放平台协议与私聊（C2C）通信。填好 `.env` 一条命令即可启动。
...
- **追加式长期记忆**：记忆一旦入库就永久保留，不再按时间遗忘，也不再分级；便宜模型只判「记 / 不记」（有了 rerank，分级已无意义），纯寒暄短消息自动跳过省 token；主模型对话并按需调用记忆工具。遗忘/取代仍走软删除保留可审计留痕。
...
- **两段式检索**：① 进程内暴力余弦召回候选（个人记忆库量级毫秒出结果），② 进程内 `Qwen3-Reranker-0.6B` 交叉编码器给 (query, 候选) 联合打分精排 top-k，精度远高于纯余弦；重排加载/推理失败会自动回退到纯余弦顺序、服务不受影响（见[数据结构](#数据结构)）。
...
- **网页抓取（内置）**：内置 `fetch_url` 工具直接拉取公开链接、抽正文转 Markdown 回传给模型，纯 Rust、无外部服务，替代 Firecrawl；只处理静态/SSR 页面，不渲染 JS（见 [网页抓取与 MCP 工具](#网页抓取与-mcp-工具)）。
...
podman exec <容器> mneme memory forget <id> <id> ... # 软删除一条或多条（active=0，库里留痕）
podman exec <容器> mneme memory forget --all --yes   # 软删除全部记忆
podman exec <容器> mneme memory forget --all --purge --yes  # 硬删除全部（彻底 DELETE，不可逆）
podman exec <容器> mneme memory stats                # 按活跃/等级/类型汇总条数，删完核对
```

> 容器名 `<容器>` 取决于你的 quadlet `ContainerName`（默认部署里是 `mneme`，即 `podman exec mneme mneme memory list`）。

`list`/`show` 只读打开（`query_only`），与运行中的服务共享同一 WAL 库、互不影响；`forget` 是写操作，靠 WAL + busy_timeout 与服务并发安全。`list` 默认文本表格每行为 `✓ 时间 L等级 kind ×重复次数 id前缀 摘要`（单用户时用户列省略、只在标题带一次 `user=`，多用户才逐行加 `[尾号]`）。`show`/`forget` 认 id 前缀（像 git 短哈希，如 `memory show 7642e9b1`），多条命中会提示多给几位。`forget` 可一次给多个 id，默认软删除（`active=0`，检索不到但库里留痕）；`--purge` 改硬删除（彻底 `DELETE` + 级联清实体链接，不可逆）；`--all` 删全部——`--all` 或 `--purge` 都必须配 `--yes` 确认，防误触（moods 情绪时间线不受影响）。`--json` 输出全字段（含完整 `id`/时间戳/`expires_at`）。`stats` 是只读汇总（活跃/失效计数、按等级与类型分布、时间跨度），只给数不给内容——看具体记忆、拿 id 去 `show`/`forget` 仍用 `list`，两者互补。
...
### 内置 fetch_url（网页抓取，替代 Firecrawl）
...
- `memories`：长期记忆——正文、`subject`（user/assistant）、float16 向量 BLOB；**追加式：不因时间过期、也不分级**（`expires_at`/`level` 列保留以兼容旧库，新记忆分别恒为 NULL / 统一值）；软删除/取代仍保留可审计留痕；
...
检索分两段：① 把该用户的全部活跃记忆向量做进程内暴力余弦（向量入库前已归一化，点积即余弦；个人记忆库量级毫秒出结果），取相似度 top-`RERANK_CANDIDATES` 候选；② 用进程内 `Qwen3-Reranker-0.6B` 交叉编码器给每个 (query, 候选正文) 联合打分，精排取 top-`MEMORY_SEARCH_LIMIT`。重排关闭、模型加载失败或推理出错时，自动回退到第 ① 段的余弦顺序，检索照常返回。返回给上层的 `score`：重排生效时是重排相关性概率，回退时是原始余弦相似度。
